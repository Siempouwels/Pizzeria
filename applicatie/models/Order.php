...
class Order extends Model
{
    public function getUserOrders(): array
    {
        if (isset($_SESSION['username'])) {
            $stmt = $this->db->prepare("SELECT * FROM Pizza_Order WHERE client_username = :username ORDER BY datetime DESC");
        ...
        if (! empty($_SESSION['order_ids'])) {
            ...
            if (! empty($ids)) {
                ...
                $stmt = $this->db->prepare("SELECT * FROM Pizza_Order WHERE order_id IN ($in) ORDER BY datetime DESC");
                $stmt->execute($ids);
                return $stmt->fetchAll();
            }
        }
    ...
    {
        $stmt = $this->db->query("SELECT * FROM Pizza_Order ORDER BY datetime DESC");
        return $stmt->fetchAll();
    }
}
